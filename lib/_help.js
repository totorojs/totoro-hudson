...
exports.generateTotoroReport = function(result) {
  ...
  style.push('.error {color:red;}')
  style.push('.info {color:green;}')
  style.push('.m31 {color:green;}')
  style.push('.m32 {color:red;}')
  style.push('.m33 {color:yellow;}')
  style.push('.log {color:grey;}')
  style.push('</style>')
...
function renderBrowserInfo(info) {
    ...
    info = info.replace(/\n/g, '<br/>') //replace('/\s/g', '&nbsp;')
    if (/^warn|debug|info/.test(info)) {
      return '<div class="log">' + info + '</div>';
    }
